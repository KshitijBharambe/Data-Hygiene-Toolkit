...
class BaseParameterSchema(BaseModel):
    ...
    columns: List[str] = Field(...,
                               description="Target columns for validation")
    ...
    def validate_columns(cls, v):
        ...
        if len(v) > 100:
            raise ValueError("Too many columns specified (max 100)")
        return v
...
class StandardizationParameters(BaseParameterSchema):
    ...
    type: str = Field(...,
                      description="Type of standardization: date, phone, email")
...
class ValueListParameters(BaseParameterSchema):
    ...
    def validate_allowed_values(cls, v):
        ...
        if len(v) > 1000:
            raise ValueError("Too many allowed values (max 1000)")
        return v
...
class CrossFieldRuleDefinition(BaseModel):
    ...
    type: str = Field(..., description="Rule type: dependency, mutual_exclusion, conditional, sum_check")
...
class CustomParameters(BaseParameterSchema):
    ...
    type: str = Field(..., description="Custom validation type: python_expression, lookup_table, custom_function")
    ...
    @validator('type')
    def validate_type(cls, v):
        allowed_types = ['python_expression',
                         'lookup_table', 'custom_function']
        if v not in allowed_types:
            raise ValueError(
                f"Type must be one of: {', '.join(allowed_types)}")
        return v
...
class ParameterValidator:
    ...
    def validate_parameters(self, rule_kind: RuleKind, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise ValueError(
                f"Parameter validation failed for {rule_kind}: {str(e)}")
