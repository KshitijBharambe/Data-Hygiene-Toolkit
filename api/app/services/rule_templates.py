...
from app.models import (
    RuleTemplate, RuleSuggestion, Rule, RuleKind, Criticality,
    Dataset, User, DatasetColumn
)
...
class RuleTemplateService:
    ...
    def apply_template(
        ...
        rule = Rule(
            ...
            target_columns=json.dumps(
                template_params.get('target_columns', [])),
    ...
    def generate_suggestions_for_dataset(
        ...
        for column in columns:
            column_suggestions = self._generate_column_suggestions(
                dataset, column)
    ...
    def _generate_column_suggestions(
        ...
        if column_type in ['int64', 'float64', 'number']:
            template = self._find_template_by_kind(
                RuleKind.statistical_outlier)
            if template:
                suggestions.append(self._create_suggestion(
                    dataset_id=dataset.id,
                    template_id=template.id,
    ...
    def _suggest_format_validation(
        ...
        # Email validation
        if any(keyword in column_name for keyword in ['email', 'mail', 'email_address']):
    ...
    def _find_template_by_kind(self, kind: RuleKind) -> Optional[RuleTemplate]:
        ...
        return self.db.query(RuleTemplate).filter(
            RuleTemplate.template_kind == kind,
            RuleTemplate.is_active == True
        ).first()
    ...
    def mark_suggestion_applied(
        ...
        suggestion = self.db.query(RuleSuggestion).filter(
