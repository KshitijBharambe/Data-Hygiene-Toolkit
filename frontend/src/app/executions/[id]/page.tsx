...
export default function ExecutionDetailPage() {
  ...
  const { data: execution, isLoading: executionLoading, error: executionError } = useExecution(executionId)
  const { data: summary, isLoading: summaryLoading } = useExecutionSummary(executionId)
  const { data: issuesData, isLoading: issuesLoading } = useExecutionIssues(executionId)
      ...
      <MainLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-red-600">
            Error loading execution: {executionError?.message || 'Execution not found'}
          </div>
        </div>
      </MainLayout>
    )
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {duration ? `${duration}s` : execution.status === 'running' ? 'Running...' : 'N/A'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{execution.total_rules || 0}</div>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between text-sm">
              <span>Started: {format(new Date(execution.started_at), 'MMM d, yyyy HH:mm:ss')}</span>
              {execution.finished_at && (
                <span>Finished: {format(new Date(execution.finished_at), 'MMM d, yyyy HH:mm:ss')}</span>
              )}
            </div>
            {execution.status === 'running' && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{formatDistanceToNow(new Date(execution.started_at), { addSuffix: true })}</span>
                </div>
        ...
        <Tabs defaultValue="summary" className="space-y-4">
            ...
            {summaryLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold">{summary.total_rows?.toLocaleString() || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Rows Affected:</span>
                      <span className="font-bold">{summary.rows_affected?.toLocaleString() || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Columns Affected:</span>
                      <span className="font-bold">{summary.columns_affected || 0}</span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    {summary.issues_by_severity && Object.keys(summary.issues_by_severity).length > 0 ? (
                      <div className="space-y-2">
                        {Object.entries(summary.issues_by_severity).map(([severity, count]) => (
                          <div key={severity} className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              <Badge className={severityColors[severity as Criticality]}>
                                {severity}
                              </Badge>
                            </div>
                    ...
                    <TableBody>
                      {issues.map((issue) => (
                        <TableRow key={issue.id}>
                          <TableCell className="font-mono">{issue.row_index + 1}</TableCell>
                          <TableCell className="font-mono">{issue.column_name}</TableCell>
          ...
          <TabsContent value="rules" className="space-y-4">
            <Card>
              ...
              <CardContent>
                {summary?.rule_performance && summary.rule_performance.length > 0 ? (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {summary.rule_performance.map((rulePerf) => (
                        <TableRow key={rulePerf.rule_id}>
                          <TableCell className="font-mono text-sm">
                            {rulePerf.rule_id.slice(0, 8)}...
                          </TableCell>
                          <TableCell>
                            <Badge variant={rulePerf.error_count > 0 ? "destructive" : "secondary"}>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium mb-2">No Rule Performance Data</h3>
                    <p className="text-muted-foreground">
                      Rule performance details are not available for this execution.
                    </p>
                  </div>
                )}
              </CardContent>
