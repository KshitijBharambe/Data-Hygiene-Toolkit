...
export default function ExecutionsPage() {
  ...
  const filteredExecutions = executionsData?.items?.filter(execution => {
  ...
  if (error) {
    return (
      <MainLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-red-600">Error loading executions: {(error as Error).message}</div>
        </div>
      </MainLayout>
    )
  }
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{executionsData?.items?.length || 0}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">
                {executionsData?.items?.filter(execution => execution.status === 'running').length || 0}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">
                {executionsData?.items?.filter(execution => execution.status === 'succeeded').length || 0}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">
                {executionsData?.items?.filter(execution => execution.status === 'failed').length || 0}
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as ExecutionStatus | 'all')}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="partially_succeeded">Partially Succeeded</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </CardHeader>
            ...
            {filteredExecutions.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground mb-4">
                  {searchTerm || statusFilter !== 'all' ? 'No executions match your criteria.' : 'Get started by running rules on your datasets.'}
                </p>
                <Button asChild>
                  <Link href="/executions/create">
                    <Play className="h-4 w-4 mr-2" />
                  ...
                  {filteredExecutions.map((execution) => {
                    ...
                    return (
                      <TableRow key={execution.id}>
                        ...
                        <TableCell>
                          {execution.total_rows ? execution.total_rows.toLocaleString() : 'N/A'}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            <div>{format(new Date(execution.started_at), 'MMM d, yyyy')}</div>
